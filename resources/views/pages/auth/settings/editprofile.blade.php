...
@section('content')
    <x-common.page-breadcrumb pageTitle="Edit Profile">
        <x-slot:breadcrumbs>
            ...
            <li>
                <span class="text-gray-700 dark:text-gray-400">Edit Profile</span>
            </li>
        </x-slot:breadcrumbs>
    </x-common.page-breadcrumb>

    <x-layouts.settings title="Edit Profile" description="Update your profile information">
        @if (session('status'))
            <div class="mb-6">
        ...
        <form method="POST" action="{{ route('settings.profile.update') }}" class="space-y-6">
            ...
            <!-- Name Input -->
            <div>
                <x-forms.input name="name" label="Name" type="text" :value="$user->name" required autofocus />
            </div>

            <!-- Email Input -->
            <div>
                <x-forms.input name="email" label="Email" type="email" :value="$user->email" required />
            </div>
